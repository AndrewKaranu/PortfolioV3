...
// ─── Main component ────────────────────────────────────────────────────────────
export function PhotobookViewer({ onBack }: Props) {
  ...
  const [loadingMessage, setLoadingMessage]   = useState("Loading photobook…");
  ...
  useEffect(() => {
    ...
    async function loadPDF() {
      try {
        ...
        const response = await fetch("/photobook.pdf");
        if (!response.ok) throw new Error(`HTTP ${response.status}: could not fetch photobook.pdf`);
        ...
        for (let pageNum = 1; pageNum <= numPages; pageNum++) {
          ...
          setLoadingMessage(`Rendering page ${pageNum} of ${numPages}…`);
          ...
          // Update incrementally so user sees pages appear
        ...
        if (!cancelled) {
          setLoadError(err instanceof Error ? err.message : "Failed to load photobook");
          setLoading(false);
        }
      }
    }
  ...
  if (loading || loadError) {
    return (
      ...
      >
        {loadError ? (
          <>
            <p style={{ fontSize: 18, marginBottom: 16, color: "rgba(255,100,100,0.9)" }}>
              {loadError}
            </p>
            <button
              onClick={onBack}
          ...
          <>
            <div style={{ fontSize: 22, fontWeight: 700, marginBottom: 8 }}>Save Our Rhinos</div>
            ...
            {/* Show pages as they render if we have any */}
            {pages.length > 0 && (
